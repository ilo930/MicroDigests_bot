(() => {
  ...
  function frame() {
    ...
    // It wanders: one slow rise and fall over the whole reel, plus a shorter
    // sideways sway so it drifts out of its column rather than running down a
    // rail. Small numbers on purpose.
    const dy = Math.sin(g * Math.PI * 2) * 26;
    const dx = Math.cos(g * Math.PI * 4) * 11;
    tin.style.transform = `translate3d(${dx.toFixed(1)}px,${dy.toFixed(1)}px,0)`;
